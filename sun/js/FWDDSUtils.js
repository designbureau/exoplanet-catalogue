 ...
 * Plasmic Audio Player Utils.
 * @author Tibi - FWDesign [https://webdesign-flash.ro/]
 * Copyright © Since 2006 All Rights Reserved.
 */

export default class FWDDSUtils{
	

	/*
     * Array.
     */
	static indexOfArray(array, prop){
	...
	static randomizeArray(aArray) {
		let randomizedArray = [];
		let copyArray = aArray.concat();
     ...
     * DOM manipulation.
     */
	static getChildById = function(id){
		return document.getElementById(id) || undefined;
	};

    static hasAttribute = function(el, attr){
		if(el.hasAttribute){
			return el.hasAttribute(attr); 
		}else {
			let test = el.attributes[attr];
			return  test ? true : false;
		}
	};

    static getChildren(el, allNodesTypes){
		let kids = [];
    ...
    static getChildrenFromAttribute(el, attr, allNodesTypes){
		let kids = [];
		for(let c = el.firstChild; c != null; c = c.nextSibling){
			if(allNodesTypes && FWDDSUtils.hasAttribute(c, attr)){
				kids.push(c);
			}else if(c.nodeType === 1 && FWDDSUtils.hasAttribute(c, attr)){
				kids.push(c);
			}
		}
    ...
    static getChildFromNodeListFromAttribute(el, attr, allNodesTypes){
		for(let c = el.firstChild; c != null; c = c.nextSibling){
			if(allNodesTypes && FWDDSUtils.hasAttribute(c, attr)){
				return c;
			}else if(c.nodeType === 1 && FWDDSUtils.hasAttribute(c, attr)){
    ...
    static getChildAt(el, index){
		let kids = FWDDSUtils.getChildren(el);
		if(index < 0) index += kids.length;
		if(index < 0) return null;
		return kids[index];
	};

    static getAttributeValue = function(el, attr){
		if(!FWDDSUtils.hasAttribute(el, attr)) return undefined;
		return el.getAttribute(attr);	
	};
	
	static insertNodeAt = function(prt, child, node){
		let children = FWDDSUtils.children(prt);
		if(node < 0 || node > children.length){
			throw new Error("invalid index!");
		}else {
			prt.insertBefore(child, children[node]);
		};
	};
     ...
     * DOM geometry.
     */
	static hitTest = function(target, x, y){
	...
	static getScrollOffsets = function(){
		// All browsers.
	...
	static getViewportSize(){
		if(FWDDSUtils.hasPointerEvent && navigator.msMaxTouchPoints > 1){
			return {w:document.documentElement.clientWidth || window.innerWidth, h:document.documentElement.clientHeight || window.innerHeight};
		}
		
		if(FWDDSUtils.isMobile) return {w:window.innerWidth, h:window.innerHeight};
		return {w:document.documentElement.clientWidth || window.innerWidth, h:document.documentElement.clientHeight || window.innerHeight};
	};
	
	static getViewportMouseCoordinates(e){
		let offsets = FWDDSUtils.getScrollOffsets();
		
		if(e.touches){
			return{
				x:e.touches[0] == undefined ? e.touches.pageX - offsets.x :e.touches[0].pageX - offsets.x,
				y:e.touches[0] == undefined ? e.touches.pageY - offsets.y :e.touches[0].pageY - offsets.y
			};
		}
		
		return{
			x: e.clientX == undefined ? e.pageX - offsets.x : e.clientX,
			y: e.clientY == undefined ? e.pageY - offsets.y : e.clientY
		};
	};
     ...
     * Browsers test.
     */
    static hasPointerEvent = (function(){
		return Boolean(window.navigator.msPointerEnabled) || Boolean(window.navigator.pointerEnabled);
	}());
	
	static isMobile = (function (){
		let agents = ['android', 'webos', 'iphone', 'ipad', 'blackberry', 'kfsowi'];
	    for(let i in agents) {
	...
	static isChrome = (function(){
		if(FWDDSUtils.isIE) return;
		let t = navigator.userAgent.toLowerCase();
		if(t.match(/browser/ig)) return;
		return t.match(/chrome/ig);
	}());
	
	static isSafari = (function(){
		return navigator.userAgent.toLowerCase().indexOf('safari') != -1 && navigator.userAgent.toLowerCase().indexOf('chrome') == -1;
	}());
	
	static isOpera = (function(){
		return navigator.userAgent.toLowerCase().indexOf('opr') != -1;
	}());
	
	static isFirefox = (function(){
		return navigator.userAgent.toLowerCase().indexOf('firefox') != -1;
	}());
	
	static isIEWebKit = (function(){
		return Boolean(document.documentElement.msRequestFullscreen);
	}());
	
    static isMac = (function(){
		return Boolean(navigator.appVersion.toLowerCase().indexOf('mac') != -1);
	}());
	
	static isWin = (function(){
		return Boolean(navigator.appVersion.toLowerCase().indexOf('win') != -1);
	}());
	
	static isIOS = (function(){
		if(navigator.platform === 'MacIntel' && navigator.maxTouchPoints > 1) return true;
		return navigator.userAgent.match(/(iPad|iPhone|iPod)/ig);
	}());

	static isIpad = (function(){
		return navigator.userAgent.match(/iPad/ig);
	}());

	
	static isIphone = (function(){
		return navigator.userAgent.match(/(iPhone|iPod)/ig);
	}());
	...
	// Get valid source.
	static getValidSource =  function(source){
	
		let firstUrlPath = source.substr(0,source.lastIndexOf("/") + 1) 
		if(!FWDDSUtils.isURLEncoded(firstUrlPath)){
		...
		if(source.match(/\.mp3|\.mp4|\.m3u8|\.txt|\.srt|\.vtt|\.jpg|\.jpeg|\.png/ig)
			...
			if(FWDDSUtils.isURLEncoded(secondUrlPath)){
		...
		return  firstUrlPath + secondUrlPath;
	}

	static isURLEncoded = function(url){
		try{
	...
	// Disable selection.
	static disableElementSelection(el){
		try{el.style.userSelect = "none";}catch(e){};
		try{el.style.MozUserSelect = "none";}catch(e){};
		try{el.style.webkitUserSelect = "none";}catch(e){};
		try{el.style.khtmlUserSelect = "none";}catch(e){};
		try{el.style.oUserSelect = "none";}catch(e){};
		try{el.style.msUserSelect = "none";}catch(e){};
		try{el.msUserSelect = "none";}catch(e){};
		e.onselectstart = function(){return false;};
	};

	// Time format.
	static setMediaType(src, obj){
		if(/\.jpg|\.webp|\.jpeg|\.png|\.bmp|\.gif|\.tif|\.tiff|\.jfi|\.jfif|\.exif|\.svg/i.test(src)){
			obj.type = 'image';
		}else if(/\.mp4|\.m3u8|\.mpd/i.test(src)){
			obj.type = 'video';
		}
	};
	...
	static getSecondsFromString(str){
		let hours = 0;
		let minutes = 0;
		let secs = 0;
		let duration = 0;
	...
	// Colors.
	static getRGBFromHex(hexColor){
		if(/^#/.test(hexColor)){
			hexColor = hexColor.substring(1);
		}
		let hexR = parseInt(hexColor.slice(0, 2), 16);
		let hexG = parseInt(hexColor.slice(2, 4), 16);
		let hexB = parseInt(hexColor.slice(4, 6), 16);
		let r = hexR / 255;
		let g = hexG / 255;
		let b = hexB / 255;
		return({r:r, g:g, b:b});
	...
	// Get URL arguments.
	static getUrlArguments(){
		let hash = window.location.hash.substr(1); 
		let hashArgs = hash.split('&');
		let values = {}; 

		// Iterate through each argument and extract the key-value pairs
		for (let i = 0; i < hashArgs.length; i++) {
			let arg = hashArgs[i].split('=');
			let key = arg[0];
			let value = arg[1];
	...
	// Add elipse if overflow.
	static addEllipsisIfOverflow(element, offsetWidth = 0) {
		let container = element.parentElement;

		if (element.offsetWidth > container.offsetWidth - offsetWidth) {
		  	let text = element.textContent;
		  	let maxLength = text.length;
		
		  	while (element.offsetWidth > container.offsetWidth - offsetWidth && maxLength > 0) {
				maxLength--;
	...
	// Check if HTML content ends with special characters.
	static checkElementContentEndsWith(element, characters) {
		let content = element.textContent || element.innerText;
	  
		let endIndex = characters.length;
		let contentEnd = content.slice(-endIndex);
	  
		return contentEnd === characters;
	}

	// Format likes.
	static formatLikes(likes) {
		if (likes > 1000) {
		  const truncatedLikes = Math.floor(likes / 1000);
		  return truncatedLikes + "K";
		}
		return likes.toString();
	}

	// Modify class property.
	static modifyClassProperty(selector, property, value) {
	...
	static reoveClass(element, className){
		if(element.classList.contains(className)) {
			element.classList.remove(className);
		}
	}
...
if(!window['FWDAnimation']){
	...
	var _fwd_fwdScope = (typeof(fwd_module) !== "undefined" && fwd_module.exports && typeof(fwd_global) !== "undefined") ? fwd_global : this || window; //helps ensure compatibility with AMD/RequireJS and CommonJS/Node
	(_fwd_fwdScope._fwd_fwdQueue || (_fwd_fwdScope._fwd_fwdQueue = [])).push( function() {
		 ...
		 * ----------------------------------------------------------------
		 */
		_fwd_fwdScope.FWDFWD_fwdDefine("FWDAnimation", ["core.FWDAnimation","core.FWDSimpleTimeline","FWDTweenLite"], function(FWDAnimation, FWDSimpleTimeline, FWDTweenLite) {
			...
			var _slice = function(a) {
				...
				FWDAnimation = function(target, duration, vars) {
					...
					this._dirty = true;
					this.render = FWDAnimation.prototype.render;
				},
			...
			p.render = function(time, suppressEvents, force) {
				...
				if (time >= totalDur - 0.0000001) {
					...
					if (duration === 0) if (this._initted || !this.vars.lazy || force) {
						...
						if (prevRawPrevTime < 0 || (time <= 0 && time >= -0.0000001) || (prevRawPrevTime === _FWDtinyNum && this.data !== "isPause")) if (prevRawPrevTime !== time) {
							force = true;
							if (prevRawPrevTime > _FWDtinyNum) {
								callback = "onReverseComplete";
							}
						}
						this._rawPrevTime = rawPrevTime = (!suppressEvents || time || prevRawPrevTime === time) ? time : _FWDtinyNum;
					}
			...
			var _getChildrenOf = function(timeline, includeTimelines) {
				...
				getAllTweens = FWDAnimation.getAllTweens = function(includeTimelines) {
					return _getChildrenOf(FWDAnimation._rootTimeline, includeTimelines).concat( _getChildrenOf(FWDAnimation._rootFramesTimeline, includeTimelines) );
				};
			...
			FWDAnimation.globalTimeScale = function(value) {
				var tl = FWDAnimation._rootTimeline,
					t = FWDTweenLite.ticker.time;
				if (!arguments.length) {
					return tl._timeScale;
				}
				value = value || _FWDtinyNum; 
				tl._startTime = t - ((t - tl._startTime) * tl._timeScale / value);
				tl = FWDAnimation._rootFramesTimeline;
				t = FWDTweenLite.ticker.frame;
				tl._startTime = t - ((t - tl._startTime) * tl._timeScale / value);
				tl._timeScale = FWDAnimation._rootTimeline._timeScale = value;
				return value;
			};
			...
			p.duration = function(value) {
				...
				return FWDAnimation.prototype.duration.call(this, value);
			};
		...
		(function() {
			...
			var _RAD2DEG = 180 / Math.PI,
				...
				_r3 = [],
				_corProps = {},
				_fwdglobals = _fwd_fwdScope.FWDFWD_fwdDefine.globals,
				...
				BezierPlugin = _fwd_fwdScope.FWDFWD_fwdDefine.plugin({
						propName: "bezier",
						priority: -1,
						version: "1.3.7",
						API: 2,
			...
			BezierPlugin._cssRegister = function() {
				...
				_internals._registerComplexSpecialProp("bezier", {parser:function(t, e, prop, cssp, pt, plugin) {
					...
					if (v.autoRotate) {
						...
						data.autoRotate = cssp._target._fwdTransform;
		 ...
		 * ----------------------------------------------------------------
		 */
		_fwd_fwdScope.FWDFWD_fwdDefine("plugins.CSSPlugin", ["plugins.TweenPlugin","FWDTweenLite"], function(TweenPlugin, FWDTweenLite) {
				...
				_convertToPixels = _internals.convertToPixels = function(t, p, v, sfx, recurse) {
					...
					if (sfx === "%" && p.indexOf("border") !== -1) {
						...
						if (horiz && sfx === "%" && CSSPlugin.cacheWidths !== false) {
							cache = node._fwdCache = node._fwdCache || {};
							cache.time = time;
				...
				_registerPluginProp = _internals._registerPluginProp = function(p) {
					if (!_specialProps[p]) {
						...
						_registerComplexSpecialProp(p, {parser:function(t, e, p, cssp, pt, plugin, vars) {
							var pluginClass = _fwdglobals.com.fwd.plugins[pluginName];
							if (!pluginClass) {
			...
			var _transformProps = ("scaleX,scaleY,scaleZ,x,y,z,skewX,skewY,rotation,rotationX,rotationY,perspective,xPercent,yPercent").split(","),
				...
				_getMatrix = function(e, force2D) {
					var tm = e._fwdTransform || new Transform(),
						rnd = 100000,
						style = e.style,
				 ...
				 * @param {boolean=} rec if true, the transform values will be recorded to the target element's _fwdTransform object, like target._fwdTransform = {x:0, y:0, z:0, scaleX:1...}
				 * @param {boolean=} parse if true, we'll ignore any _fwdTransform values that already exist on the element, and force a reparsing of the css (calculated style)
				 * @return {object} object containing all of the transform properties/values like {x:0, y:0, z:0, scaleX:1...}
				 */
				_getTransform = _internals.getTransform = function(t, cs, rec, parse) {
					if (t._fwdTransform && rec && !parse) {
						return t._fwdTransform; //if the element already has a _fwdTransform, use that. Note: some browsers don't accurately return the calculated style for the transform (particularly for SVG), so it's almost always safest to just use the values we've already applied rather than re-parsing things.
					}
					var tm = rec ? t._fwdTransform || new Transform() : new Transform(),
					...
					if (rec) {
						t._fwdTransform = tm; //record to the object's _fwdTransform which we use so that tweens can control individual properties independently (we need all the properties to accurately recompose the matrix in the setRatio() method)
				...
				_setIETransformRatio = function(v) {
					var t = this.data, //refers to the element's _fwdTransform object
			...
			_registerComplexSpecialProp("transform,scale,scaleX,scaleY,scaleZ,x,y,z,rotation,rotationX,rotationY,rotationZ,skewX,skewY,shortRotation,shortRotationX,shortRotationY,shortRotationZ,transformOrigin,svgOrigin,transformPerspective,directionalRotation,parseTransform,force3D,skewType,xPercent,yPercent,smoothOrigin", {parser:function(t, e, parsingProp, cssp, pt, plugin, vars) {
				...
				var originalGSTransform = t._fwdTransform,
					style = t.style,
					min = 0.000001,
				...
				if (orig || (_supports3D && has3D && m1.zOrigin)) { //if anything 3D is happening and there's a transformOrigin with a z component that's non-zero, we must ensure that the transformOrigin's z-component is set to 0 so that we can manually do those calculations to get around Safari bugs. Even if the user didn't specifically define a "transformOrigin" in this particular tween (maybe they did it via css directly).
					if (_transformProp) {
						...
						if (_supports3D) {
							...
							pt = new CSSPropTween(m1, "zOrigin", 0, 0, pt, -1, pt.n); //we must create a CSSPropTween for the _fwdTransform.zOrigin so that it gets reset properly at the beginning if the tween runs backward (as opposed to just setting m1.zOrigin here)
							pt.b = copy;
			...
			var _removeProp = function(s, p) {
				...
				_setClassNameRatio = function(v) {
					...
					if (v === 1 || v === 0) {
						...
						if (v === 1 && this.t._fwdClassPT === this) {
			...
			var _setClearPropsRatio = function(v) {
				if (v === 1 || v === 0) if (this.data._totalTime === this.data._totalDuration && this.data.data !== "isFromStart") { //this.data refers to the tween. Only clear at the END of the tween (remember, from() tweens make the ratio go from 1 to 0, so we can't just check that and if the tween is the zero-duration one that's created internally to render the starting values in a from() tween, ignore that because otherwise, for example, from(...{height:100, clearProps:"height", delay:1}) would wipe the height at the beginning of the tween and after 1 second, it'd kick back in).
					...
					if (clearTransform) {
						...
						transform = this.t._fwdTransform;
						if (transform) {
							if (transform.svg) {
			...
			p._enableTransforms = function(threeD) {
				this._transform = this._transform || _getTransform(this._target, _cs, true); //ensures that the element has a _fwdTransform property with the appropriate values.
	 ...
	 * ----------------------------------------------------------------
	 */
		_fwd_fwdScope.FWDFWD_fwdDefine("easing.Back", ["easing.Ease"], function(Ease) {
			
			var w = (_fwd_fwdScope.FWDGlobals || _fwd_fwdScope),
				gs = w.com.fwd,
				_2PI = Math.PI * 2,
				_HALF_PI = Math.PI / 2,
				_class = gs._class,
	...
	if (_fwd_fwdScope.FWDFWD_fwdDefine) { _fwd_fwdScope._fwd_fwdQueue.pop()(); } //necessary in case FWDTweenLite was already loaded separately.
	...
	(function(window, moduleName) {
			...
			"use strict";
			var _exports = {},
				_fwdglobals = window.FWDGlobals = window.FWDGlobals || window;
			...
			var _namespace = function(ns) {
				...
				Definition = function(ns, FWDdependencies, func, fwd_global) {
					...
					this.check = function(init) {
						...
						if (missing === 0 && func) {
							...
							if (fwd_global) {
								_fwdglobals[n] = _exports[n] = cl; //provides a way to avoid fwd_global namespace pollution. By default, the main classes like FWDTweenLite, Power1, Strong, etc. are added to window unless a FWDGlobals is defined. So if you want to have things added to a custom object instead, just do something like window.FWDGlobals = {} before loading any FWDAnimation files. You can even set up an alias like window.FWDGlobals = windows.gs = {} so that you can access everything like gs.FWDTweenLite. Also remember that ALL classes are added to the window.com.fwd object (in their respective packages, like com.fwd.easing.Power1, com.fwd.FWDTweenLite, etc.)
								hasModule = (typeof(fwd_module) !== "undefined" && fwd_module.exports);
								if (!hasModule && typeof(define) === "function" && define.amd){ //AMD
									define((window.FWDAMDPath ? window.FWDAMDPath + "/" : "") + ns.split(".").pop(), [], function() { return cl; });
				...
				FWDFWD_fwdDefine = window.FWDFWD_fwdDefine = function(ns, FWDdependencies, func, fwd_global) {
					return new Definition(ns, FWDdependencies, func, fwd_global);
				},
	
				//a quick way to create a class that doesn't have any FWDdependencies. Returns the class, but first registers it in the FWDAnimation namespace so that other classes can grab it (other classes might be dependent on the class).
				_class = gs._class = function(ns, func, fwd_global) {
					func = func || function() {};
					FWDFWD_fwdDefine(ns, [], function(){ return func; }, fwd_global);
					return func;
				};
	
			FWDFWD_fwdDefine.globals = _fwdglobals;
	 ...
	 * ----------------------------------------------------------------
	 */
			var FWDAnimation = _class("core.FWDAnimation", function(duration, vars) {
					this.vars = vars = vars || {};
			...
			_ticker = FWDAnimation.ticker = new gs.Ticker();
			p = FWDAnimation.prototype;
	 ...
	 */
			var FWDSimpleTimeline = _class("core.FWDSimpleTimeline", function(vars) {
				FWDAnimation.call(this, 0, vars);
				this.autoRemoveChildren = this.smoothChildTiming = true;
			});
	
			p = FWDSimpleTimeline.prototype = new FWDAnimation();
	 ...
	 */
			var FWDTweenLite = _class("FWDTweenLite", function(target, duration, vars) {
					FWDAnimation.call(this, duration, vars);
			...
			p = FWDTweenLite.prototype = new FWDAnimation();
	...
	//----FWDTweenLite defaults, overwrite management, and root updates ----------------------------------------------------
			...
			var _lazyTweens = [],
				...
				_internals = FWDTweenLite._internals = {isArray:_FWDisArray, isSelector:_FWDisSelector, lazyTweens:_lazyTweens, blobDif:_blobDif}, //gives us a way to expose certain private values to other FWDAnimation classes without contaminating tha main FWDTweenLite object.
				...
				_rootFramesTimeline = FWDAnimation._rootFramesTimeline = new FWDSimpleTimeline(),
				_rootTimeline = FWDAnimation._rootTimeline = new FWDSimpleTimeline(),
				_nextGCFrame = 30,
			...
			FWDAnimation._updateRoot = FWDTweenLite.render = function() {
					var i, a, p;
			...
			_ticker.addEventListener("tick", FWDAnimation._updateRoot);
	
			var _register = function(target, tween, scrub) {
					var id = target._fwdTweenID, a, i;
					if (!_tweenLookup[id || (target._fwdTweenID = id = "t" + (_tweenLookupNum++))]) {
	...
	//---- FWDTweenLite instance methods -----------------------------------------------------------------------------
			...
			p._initProps = function(target, propLookup, siblings, overwrittenProps, index) {
				...
				if (_lazyLookup[target._fwdTweenID]) {
				...
				if (this._firstPT) if ((this.vars.lazy !== false && this._duration) || (this.vars.lazy && !this._duration)) { //zero duration tweens don't lazy render by default; everything else does.
					_lazyLookup[target._fwdTweenID] = true;
				}
				return initPlugins;
			};
			...
			p.invalidate = function() {
				...
				FWDAnimation.prototype.invalidate.call(this);
				if (this.vars.immediateRender) {
					this._time = -_FWDtinyNum; //forces a render without having to set the render() "force" parameter to true because we want to allow lazying by default (using the "force" parameter always forces an immediate full render)
					this.render(Math.min(0, -this._delay)); //in case delay is negative.
				}
			...
			p._enabled = function(enabled, ignoreTimeline) {
				...
				FWDAnimation.prototype._enabled.call(this, enabled, ignoreTimeline);
	 ...
	 */
			...
			FWDFWD_fwdDefine.plugin = function(config) {
			...
			if (a) {
				...
				for (p in _defLookup) {
					if (!_defLookup[p].func) {
						window.console.log("FWDAnimation encountered missing dependency: " + p);
					}
				}
			}
